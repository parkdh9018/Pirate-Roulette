...
void GL_VIEW::UpdateCameraPos(float dt)
{
	float r = 1.5f;
	float speed = 1.0f;

	if (dt*speed <= 36.0f) {
		cameraPos.z = r*cos(glm::radians(dt*speed)) - r;
		cameraPos.x = r*sin(glm::radians(dt*speed));
	}
}
...
void GL_VIEW::ProcessMouseMotion(int x, int y)
{
	...
	if (is_trackball_active_ || is_dolly_active_ || is_pan_active_ || is_mouse_move)
	{
		dx_ = x - x_;
		dy_ = y - y_;
...
void GL_VIEW::UpdateTrackball()
{
	...
	rot_incr_ = glm::angleAxis(angle, axis_in_camera_coord);		// rotation quarternion
...
{
	glm::vec3 v(dx_, -dy_, 0); //change vector is on the window, so v_z is 0

	pan_ += v * pan_scale_; //move all object by v.
}
...
void GL_VIEW::Update() 
{
	...
	if (is_mouse_move)			MouseMovement();
}
...
const glm::mat4 GL_VIEW::GetWorldViewMatrix() const
{
	...
	return projection_ * v * glm::translate(pan_) * glm::translate(dolly_)
		   * glm::mat4_cast(rot_)  ;
}
